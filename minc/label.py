...
class Label(Image):
    def __init__(
        self, fname=None, data=None, autoload=True, dtype=np.int32, *args, **kwargs
    ):
        self.include_not_label = False
        super(Label, self).__init__(
            fname=fname, data=data, autoload=autoload, dtype=dtype, *args, **kwargs
        )
    ...
    def get_TP(self, reference, tp_min_size=1, voxelWise=False):
        ...
        return dict(
            (k, v["regions"]) for k, v in stats["self"].items() if v["type"] == "TP"
        )
    ...
    def get_FP(self, reference, tp_min_size=1, voxelWise=False):
        ...
        return dict((k, None) for k, v in stats["self"].items() if v["type"] == "FP")
    ...
    def get_FN(self, reference, tp_min_size=1, voxelWise=False):
        ...
        return dict(
            (k, None) for k, v in stats["reference"].items() if v["type"] == "FN"
        )
    ...
    def get_TN(self, reference, tp_min_size=1, voxelWise=False):
        ...
        return dict(
            (k, v["regions"]) for k, v in stats["self"].items() if v["type"] == "TN"
        )
    ...
    def dilation(self):
        ...
        struct[2] = False
        l = copy.copy(self)
        l.data = ndimage.grey_dilation(
            self.data, size=(3, 3, 3), footprint=struct
    ...
    def binary_dilation(self, struct=None, mask=None, iterations=1):
        ...
        dilatedLabel.data = ndimage.binary_dilation(
            self.data, structure=struct, iterations=iterations, mask=mask
        ).astype(self.data.dtype)
        return dilatedLabel
    ...
    def binary_erosion(self, struct=None, mask=None, iterations=1):
        ...
        erodedLabel.data = ndimage.binary_erosion(
            self.data, structure=struct, iterations=iterations, mask=mask
        ).astype(self.data.dtype)
        return erodedLabel
