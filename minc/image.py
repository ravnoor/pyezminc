...
def display(images, label=None, imitate=None):
    ...
    if label:
        tmp_label = system.create_tmp_filename(
            prefix="tmp_" + label.name.basename(), remove=False
        )
    try:
...
class Image(object):
    ...
    def __init__(
        self, fname=None, data=None, autoload=True, dtype=np.float64, *args, **kwargs
    ):
        self.dtype = dtype
        if pyezminc is None:
    ...
    def _load(self, name, with_nan=False, metadata_only=False, *args, **kwargs):
        ...
        if not metadata_only and np.any(self.data == -sys.float_info.max):
            if not with_nan:
                ...
                self.data = np.where(
                    self.data == -sys.float_info.max, np.nan, self.data
                ).astype(self.dtype)
    ...
    def _save(self, name=None, imitate=None, history=None, *args, **kwargs):
        ...
        if not os.path.isfile(imitate):
            raise Exception(
                "Cannot imitate from non existing file: '{0}'".format(imitate)
            )
        ...
        if self.dtype != self.data.dtype:
            raise Exception(
                "Cannot save image because non consistent dtype, '{0}' != '{1}'".format(
        ...
        self.__wrapper.save(
            name, imitate=imitate, dtype=self.dtype, history=self.history
        )
    ...
    def spacing(self):
        return [
            self.__wrapper.nspacing(i) for i in range(1, self.__wrapper.nb_dim() + 1)
        ]
    ...
    def get_affine_xyz(self):
        ...
        affine_matrix = np.array(
            [
                [
                    self.spacing()[0] * cosines_transpose[0, 0],
                    self.spacing()[1] * cosines_transpose[0, 1],
                    self.spacing()[2] * cosines_transpose[0, 2],
                    sum(s * c for s, c in zip(self.start(), cosines_transpose[0])),
                ],
                [
                    self.spacing()[0] * cosines_transpose[1, 0],
                    self.spacing()[1] * cosines_transpose[1, 1],
                    self.spacing()[2] * cosines_transpose[1, 2],
                    sum(s * c for s, c in zip(self.start(), cosines_transpose[1])),
                ],
                [
                    self.spacing()[0] * cosines_transpose[2, 0],
                    self.spacing()[1] * cosines_transpose[2, 1],
                    self.spacing()[2] * cosines_transpose[2, 2],
                    sum(s * c for s, c in zip(self.start(), cosines_transpose[2])),
                ],
                [0, 0, 0, 1],
            ]
        )
    ...
    def field_strength(self, field="dicom_0x0018:el_0x0087"):
        ...
        try:
            ...
            raise Exception(
                'Could not extract the field strength as a float value: "{0}"'.format(
                    val
                )
            )
        return float(val)
    ...
    def acquisition_date(self):
        ...
        for field in field_datetime:
            try:
                ...
                m = re.search(
                    "(?P<year>[0-9]{4})(?P<month>[0-9]{2})(?P<day>[0-9]{2})\s+(?P<hour>[0-9]{2})(?P<minute>[0-9]{2})(?P<second>[0-9]{2})\.?(?P<microsecond>[0-9]+)?",
                    out,
                ).groupdict()
            except Exception as e:
                continue
        ...
        for field in field_date:
            try:
                out = mincinfo(header, field)
                m = re.search(
                    "(?P<year>[0-9]{4})(?P<month>[0-9]{2})(?P<day>[0-9]{2})", out
                ).groupdict()
            except Exception as e:
                continue
            for k in m.keys():
        ...
        for field in field_time:
            try:
                out = mincinfo(header, field)
                m = re.search(
                    "(?P<hour>[0-9]{2})(?P<minute>[0-9]{2})(?P<second>[0-9]{2})\.?(?P<microsecond>[0-9]+)?",
                    out,
                ).groupdict()
            except Exception as e:
                continue
